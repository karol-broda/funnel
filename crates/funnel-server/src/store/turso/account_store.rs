...
fn row_to_account(row: &turso::Row) -> Result<Account, StoreError> {
    Ok(Account {
        ...
        provider: row.get::<String>(2).map_err(map_err)?,
        provider_account_id: row.get::<String>(3).map_err(map_err)?,
        metadata: serde_json::from_str(&row.get::<String>(4).map_err(map_err)?)
            .map_err(|e| StoreError::Other(format!("invalid json: {e}")))?,
        created_at: parse_dt(&row.get::<String>(5).map_err(map_err)?)?,
        updated_at: parse_dt(&row.get::<String>(6).map_err(map_err)?)?,
    })
}
        ...
        Box::pin(async move {
            ...
            let mut rows = conn
                .query(
                    ...
                    turso::params![provider, provider_account_id],
                )
                .await
                .map_err(map_err)?;

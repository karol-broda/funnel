...
function DriftField() {
  ...
  useEffect(() => {
    ...
    let darkVal = document.documentElement.classList.contains("dark")
      ? 1.0
      : 0.0;
    let darkTarget = darkVal;
    const obs = new MutationObserver(() => {
      darkTarget = document.documentElement.classList.contains("dark")
        ? 1.0
        : 0.0;
    });
    ...
    const tick = () => {
      ...
      for (const p of particles) {
        ...
        const angle =
          simplex2d(tx * 0.001 + time, ty * 0.001 + time * 0.3) * Math.PI * 2.2;
        const fx = Math.cos(angle) * 0.35;
        const fy = Math.sin(angle) * 0.35;

        p.vx = p.vx * 0.97 + fx * 0.03;
        p.vy = p.vy * 0.97 + fy * 0.03;
    ...
    const draw = () => {
      ...
      for (const p of particles) {
        ...
        for (let i = 0; i < TRAIL; i++) {
          ...
          const tf = 1 - i / TRAIL;
          const a = base * tf * tf * (darkVal > 0.5 ? 0.4 : 0.25);
          if (a < 0.01) continue;
      ...
      if (vi > 0) {
        gl.bufferSubData(
          gl.ARRAY_BUFFER,
          0,
          vertexData.subarray(0, vi * FLOATS_PER),
        );
...
export default function NotFound() {
  return (
    <main className="relative flex min-h-dvh flex-col items-center justify-center overflow-hidden bg-fd-background">
      <DriftField />
      ...
      <div className="relative z-10 text-center">
        ...
        <Link
          href="/"
          className="mt-8 inline-flex items-center gap-2 rounded-full bg-fd-foreground/[0.06] px-6 py-2.5 text-sm font-medium text-fd-foreground/70 ring-1 ring-inset ring-fd-foreground/[0.08] backdrop-blur transition-all hover:bg-fd-foreground/[0.12] hover:text-fd-foreground/90"
        >
          Back to home
        </Link>
      </div>

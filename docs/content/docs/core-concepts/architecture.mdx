...
1.  **connection**: the client initiates a websocket connection to the server's `/ws` endpoint, providing its desired tunnel id in the query parameters (`/ws?id=my-tunnel`).
2.  **registration**: the server validates the id. if it's available, it creates a new `tunnel` instance and associates it with the websocket connection.
3.  **request proxying**:

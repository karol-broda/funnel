...
**access** applies optional access control, enforced by the server at the tunnel edge for `http` tunnels. `basic_auth` holds `user:pass` credentials, `allow_ip` is a list of cidr ranges permitted to connect, and `expires_secs` closes the tunnel after the given number of seconds. Invalid cidr entries fail registration with `bad_request`.

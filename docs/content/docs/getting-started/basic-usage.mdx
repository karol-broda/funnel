...
<Callout title="configuration (optional)" intent="tip">
...
<Callout title="client authentication" type="error">
for client authentication, currently there is none.
what i recommend is throwing a uuid at a subdomain and hoping it sticks like spaghetti on the wall. well yea funnel uses letsencrypt for the domain, so crt.sh indexes it faster than i can say "oops i committed my api keys again".
...
<Callout title="with configuration" intent="tip">
